...
describe Lumberjack::LogEntry do
  ...
  it "should have a severity" do
    ...
    expect(entry.severity).to eq(Lumberjack::Severity::INFO)
    entry.severity = Lumberjack::Severity::WARN
    expect(entry.severity).to eq(Lumberjack::Severity::WARN)
  end
  ...
  it "should get the severity as a string" do
    expect(Lumberjack::LogEntry.new(Time.now, Lumberjack::Severity::DEBUG, "test", "app", 1500, nil).severity_label).to eq("DEBUG")
    expect(Lumberjack::LogEntry.new(Time.now, Lumberjack::Severity::INFO, "test", "app", 1500, nil).severity_label).to eq("INFO")
    expect(Lumberjack::LogEntry.new(Time.now, Lumberjack::Severity::WARN, "test", "app", 1500, nil).severity_label).to eq("WARN")
    expect(Lumberjack::LogEntry.new(Time.now, Lumberjack::Severity::ERROR, "test", "app", 1500, nil).severity_label).to eq("ERROR")
    expect(Lumberjack::LogEntry.new(Time.now, Lumberjack::Severity::FATAL, "test", "app", 1500, nil).severity_label).to eq("FATAL")
    expect(Lumberjack::LogEntry.new(Time.now, -1, "test", "app", 1500, nil).severity_label).to eq("UNKNOWN")
  ...
  it "should be converted to a string" do
    ...
    entry = Lumberjack::LogEntry.new(t, Lumberjack::Severity::INFO, "test", "app", 1500, "ABCD")
    expect(entry.to_s).to eq("[2011-01-29T12:15:32.001 INFO app(1500) #ABCD] test")
  end
