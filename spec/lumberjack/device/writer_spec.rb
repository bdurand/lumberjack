...
RSpec.describe Lumberjack::Device::Writer do
  ...
  let(:entry) { Lumberjack::LogEntry.new(time, Logger::INFO, "test message", "app", 12345, "test" => "ABCD") }
  ...
  it "should write entries out to the stream with a default template" do
    ...
    device.write(entry)
    device.flush
    expect(stream.string).to eq("[2011-01-15T14:23:45.123 INFO app(12345)] test message [test:ABCD]#{Lumberjack::LINE_SEPARATOR}")
  end
  ...
  it "should be able to specify the time format for the template" do
    ...
    device.write(entry)
    device.flush
    expect(stream.string).to eq("[2011-01-15T14:23:45.123000 INFO app(12345)] test message [test:ABCD]#{Lumberjack::LINE_SEPARATOR}")
  end
  ...
  describe "multi line messages" do
    ...
    let(:entry) { Lumberjack::LogEntry.new(time, Logger::INFO, message, "app", 12345, test: "ABCD") }

    it "should have a default template for multiline messages" do
      device = Lumberjack::Device::Writer.new(stream)
      device.write(entry)
      device.flush
      expect(stream.string.split(Lumberjack::LINE_SEPARATOR)).to eq(["[2011-01-15T14:23:45.123 INFO app(12345)] line 1 [test:ABCD]", "> line 2", "> line 3"])
    end
    ...
    it "should be able to specify a template for multiple line messages" do
      ...
      device.write(entry)
      device.flush
      expect(stream.string).to eq("[2011-01-15T14:23:45.123 INFO app(12345)] line 1 [test:ABCD] // line 2 // line 3#{Lumberjack::LINE_SEPARATOR}")
    end
  end
end
